...
static void cr_assert_ft_putchar_fd(char *c, int fd)
{
	char *out, *err;
	...
	if (fd == 1)
	{
		cr_assert_str_eq(out, c);
		cr_assert_str_eq(err, "");
	}
	else if (fd == 2)
	{
		cr_assert_str_eq(out, "");
		cr_assert_str_eq(err, c);
	}
	free(out);
	free(err);
...
Test(ft_putchar_fd, a)
{
	cr_assert_ft_putchar_fd("a", 1);
}

Test(ft_putchar_fd, U)
{
	cr_assert_ft_putchar_fd("U", 1);
}

Test(ft_putchar_fd, a_stderr)
{
	cr_assert_ft_putchar_fd("a", 2);
}
